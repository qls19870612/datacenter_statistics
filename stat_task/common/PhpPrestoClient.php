...
class PhpPrestoClient {
     ...
     * 				- Catalog
     */
    function __construct($connectUrl, $catalog, $schema) {
        $this->url = $connectUrl;
    ...
    private function GetVarFromResult() {
        ...
        if (isset($decodedJson->{'data'})) {
            $columns_obj_array = $decodedJson->columns;
            $colums_name_array = array();
            foreach ($columns_obj_array as $columns_obj) {
                $colums_name_array[] = $columns_obj->name;
            }
            $return_data = $decodedJson->{'data'};
            foreach ($return_data as $data_index => $vals) {
                $filed_data_array = array();
                foreach ($vals as $key => $val) {
                    $filed_data_array[$colums_name_array[$key]] = $val;
                }
                $return_data[$data_index] = $filed_data_array;
            }
            $this->data = array_merge($this->data, $return_data);
        }
